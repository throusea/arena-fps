...
/** Replicated timing data for an automatic magazine reload. */
USTRUCT(BlueprintType)
struct ARENA_API FNetWeaponReloadState
{
	GENERATED_BODY()
	...
	/** Reload completion time in the server world clock. */
	UPROPERTY(BlueprintReadOnly, Category="Weapon|Reload")
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FNetWeaponReloadStateChangedSignature, bool, bIsReloading);
...
public:
	...
	/** Reliably reports server-confirmed damage to this weapon's owning client. */
	UFUNCTION(Client, Reliable)
	void ClientConfirmHit(const FNetWeaponImpactResult& ImpactResult);
	...
	bool IsReloading() const { return ReloadState.bIsReloading; }

	/** Returns normalized reload completion in the range [0, 1]. */
	UFUNCTION(BlueprintPure, Category="Weapon|Reload")
	...
	FNetWeaponReloadStateChangedSignature OnReloadStateChanged;
...
protected:
	...
	UFUNCTION(BlueprintImplementableEvent, Category="Weapon|Reload", meta=(DisplayName="Weapon Reload State Changed"))
	void BP_OnReloadStateChanged(bool bNewIsReloading);
...
protected:
	...
	/** Uses a finite magazine with unlimited reserve ammunition. */
	...
	/** Time required to refill an empty magazine from the unlimited reserve. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Weapon|Reload", meta=(ClampMin=0.01, Units="s"))
	float ReloadDuration = 2.0f;

	UPROPERTY(VisibleInstanceOnly, ReplicatedUsing=OnRep_ReloadState, BlueprintReadOnly, Category="Weapon|Reload")
	FNetWeaponReloadState ReloadState;
